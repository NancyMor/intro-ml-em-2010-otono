...
## Modelación aditiva por pasos (FSAM)
...
de modelado por estapas (Forward stagewise additive modeling, FSAM).
...
dond las $T_k$ son árboles individuales. Aunque parece similar a 
bagging o bosques aleatorios, la diferencia está en cómo buscamos ajuster este modelo.
Buscamos minimizar
...
Nótese que queremos minimizar la pérdida sobre una suma arbitraria de árboles.
Este puede ser un problema difícil, dependiendo de la familia 
que usemos para los árboles $T_k$, y sería difícil resolver por fuerza bruta. 
...
intentamos resolver el problema más simple de añadir un término adicional
...
del paso anterior**, y podemos usar CART para resolverlo: ajustamos un árbol
de regresión con las mismas covariables, pero ahora la respuesta son los residuales
del paso anterior.

Otra manera de decir esto es que añadimos un término adicional
...
library(rpart)
x <- rnorm(200, 0, 30)
y <- 2*ifelse(x < 0, 0, sqrt(x)) + rnorm(200, 0, 1)
dat <- tibble(x = x, y = y)
```
...
de entrenamiento están ponderados por $d_{m,i}$).

Una solución para resolver aproximadamente este problema de minimización, es **gradient boosting**.
...
queremos agregar una cantidad $z_i$
tal que el valor de la pérdida
$$L(y, f_{m-1}(x^{(i)})+z_i)$$
se reduzca. Entonces sabemos que podemos mover la z en la dirección opuesta al gradiente

$$z_i = -\frac{\partial L}{\partial z}(y^{(i)}, f_{m-1}(x^{(i)}))$$

Si definimos nuestro nuevo predictor $f_m$ con $f_m(x^{(i)}) = f_{m-1}(x^{(i)}) + z_i$,
**tenemos el problema de que el nuevo predictor sólo está definido en los puntos de enrenamiento**.
Esto es un problema, pues queremos predecir en cualquier $x$ con $f_m(x)$.

La solución de gradient boosting es encontrar un árbol $T(x)$, definida para toda $x$ tal que

$$T(x^{(i)})\approx z_i,$$
es decir, queremos resolver un problema de regresión con un árbol donde la nueva respuesta
es $z_i$ (el gradiente en el caso $i$). Usamos pérdida cuadrática para resolver:

e intentar resolver
\begin{equation}
\min_T \sum_{i=1}^n (z_i - T(x^{(i)}))^2,
(\#eq:min-cuad-boost)
\end{equation}
...
podríamos escoger $\nu$ (tamaño de paso o tasa de aprendizaje) suficientemente chica y movemos
nuestro predictor anterior $f_{m-1}$ en las direcciones que el árbol indica: 
